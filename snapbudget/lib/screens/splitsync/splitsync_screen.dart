...
class _SplitSyncScreenState extends State<SplitSyncScreen>
            ...
            if (owes)
              GestureDetector(
                onTap: () => _showRemindDrawer(
                  friendName: f['name'] as String,
                  amount: f['amount'] as String,
                  billTitle: 'Shared Bill',
                ),
                child: Container(
                  padding:
  ...
  ///   1. Remind Myself  – schedule a local notification
  ///   2. Remind My Friend – share a payment request message
...
// =============================================================================
// REMIND DRAWER
// A tabbed bottom‑sheet drawer with two sections:
//   1. Remind Myself  – schedule a local notification so YOU remember to collect
//   2. Remind My Friend – share a payment request via any installed app
// =============================================================================

class _RemindDrawer extends StatefulWidget {
  /// The friend's display name.
  final String friendName;

  /// Amount owed, e.g. "₹3,125".
  final String amount;

  /// Short label for the shared expense, e.g. "Dinner".
  ...
  @override
  State<_RemindDrawer> createState() => _RemindDrawerState();
}

class _RemindDrawerState extends State<_RemindDrawer>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;
  ...
  // ---------------------------------------------------------------------------
  // Remind Myself helpers
  // ---------------------------------------------------------------------------

  Future<void> _scheduleIn(int days) async {
    ...
    try {
      ...
      if (mounted) {
        Navigator.pop(context);
        _toast('⏰ Reminder set for ${_fmtDate(target)}', AppTheme.successGreen);
      }
    } catch (e) {
      if (mounted) _toast('Failed: $e', AppTheme.errorRed);
    } finally {
      if (mounted) setState(() => _scheduling = false);
    }
  }

  Future<void> _scheduleCustom() async {
    ...
    if (target.isBefore(now)) {
      _toast('Please pick a future date & time.', AppTheme.errorRed);
      return;
    }
  ...
  // ---------------------------------------------------------------------------
  // Remind My Friend helpers
  // ---------------------------------------------------------------------------

  String get _shareText => 'Hey ${widget.friendName}! 👋\n\n'
      'Just a friendly reminder – you owe me ${widget.amount} '
      'for "${widget.billTitle}".\n\n'
      'Please settle up at your earliest convenience. Thank you! 🙏\n\n'
      '— Sent via SnapBudget';
  ...
  Future<void> _shareNow() async {
    try {
      ...
      _toast('Failed to share: $e', Colors.red);
    }
  }
  ...
  String _fmtDate(DateTime dt) => '${dt.day}/${dt.month}/${dt.year} at '
      ...
      child: SafeArea(
        ...
        child: Column(
          ...
          children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Remind',
                      style: GoogleFonts.inter(
                          ...
                          fontWeight: FontWeight.w800,
                          color: AppTheme.textDark)),
                  Text('${widget.friendName} • ${widget.amount}',
                      style: GoogleFonts.inter(
                          fontSize: 13, color: AppTheme.textMedium)),
                ]),
              ]),
            ...
            // Tab content — Flexible lets the sheet grow to fit content
            Flexible(
              child: TabBarView(
                controller: _tabController,
                children: [
  ...
  Widget _buildRemindMyselfTab() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          'Schedule a notification so you remember to collect '
          '${widget.amount} from ${widget.friendName}.',
          style: GoogleFonts.inter(
        ...
        if (_scheduling)
          ...
          Column(children: [
            _optionTile(
              ...
              label: 'In 1 day',
              subtitle: _fmtDate(DateTime.now().add(const Duration(days: 1))),
              onTap: () => _scheduleIn(1),
            ),
            const SizedBox(height: 10),
            _optionTile(
              ...
              label: 'In 2 days',
              subtitle: _fmtDate(DateTime.now().add(const Duration(days: 2))),
              onTap: () => _scheduleIn(2),
            ),
            const SizedBox(height: 10),
            _optionTile(
              ...
              subtitle: 'Pick a specific moment',
              onTap: _scheduleCustom,
            ),
          ]),
      ]),
    );
  ...
  Widget _buildRemindFriendTab() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          'Send a payment reminder to ${widget.friendName} via WhatsApp, SMS, or any messaging app.',
          style: GoogleFonts.inter(
              fontSize: 13, color: AppTheme.textMedium, height: 1.5),
        ),
        ...
        Container(
          ...
          child: Text(_shareText,
              style: GoogleFonts.inter(
                  fontSize: 12, color: AppTheme.textDark, height: 1.6)),
        ),
        const SizedBox(height: 16),
